...
<main class="px-5">
    ...
    <div class="row tksl">
        ...
        <div class="col-lg m-2 border bg-warning rounded">
            ...
            <h5 class="m-2 text-white">Tài khoản</h5>
    ...
    <div style="margin: 10px 35px; display: grid; grid-template-columns: 3fr 1fr;">
        <h3>Thống Kê</h3>
...
<script>
    ...
    function drawChart() {
        ...
        const data1 = google.visualization.arrayToDataTable([
            ['Trạng Thái', 'Số Lượng'],
            <?php foreach ($trangthaidonhang as $data) : extract($data); ?>
                ['<?= $trangthai ?>', <?= $soluong ?>],
            <?php endforeach; ?>
        ]);
        // Set Data 2
        const data2 = google.visualization.arrayToDataTable([
            ['Thống Kê', 'Số Lượng'],
            <?php foreach ($donhangdaban as $data) : extract($data); ?>
                ['<?= date("Y-m-d", strtotime($ngaydat)) ?>', <?= $soluongoder ?>],
            <?php endforeach; ?>
        ]);
        // Set Data 3
        const data3 = google.visualization.arrayToDataTable([
            ['Thống Kê', 'Số Lượng'],
            <?php foreach ($sanphamdaban as $data) : extract($data); ?>
                ['<?= date("Y-m-d", strtotime($ngaydat)) ?>', <?= $sanphamdaban ?>],
            <?php endforeach; ?>
        ]);
